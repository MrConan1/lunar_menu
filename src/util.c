...
/* Global Basename */
static char G_BaseName[300];

/*************************************************************/
/* setBaseName - Creates a basename by removing everything   */
/*               after the period of the passed in name.     */
/*************************************************************/
void setBaseName(char* basename){
	
	int x;
	int len = strlen(basename);
	memset(G_BaseName, 0, 300);
	
	for (x = 0; x < len; x++){
		if (basename[x] == '.')
			break;
		else if ((basename[x] == '\\') || (basename[x] == '/'))
			G_BaseName[x] = '_';
		else
			G_BaseName[x] = basename[x];
	}
